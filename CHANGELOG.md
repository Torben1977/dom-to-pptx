...
- `onBoundaryFindings` option: called with the findings whose subtree `rasterize` actually replaced, so a caller that trades a failed export for a picture can still say what stopped being editable text. `exportHtmlToPptx` carries it across the page boundary, where a callback cannot travel.
- Boundary finding `float-in-text-flow`: a float shortens the lines beside it and leaves the ones below it full width, which one PowerPoint text rectangle cannot express. Asked of the float and of the block it is laid out in, at any nesting depth on either side. A float that only places blocks beside each other carries no text across the float and is not a finding; neither is an absolutely positioned box, which is painted beside the text without moving it, nor the slide itself, which is the canvas rather than an object on it.
...
- A rasterization that fails no longer drops the object without a trace. The queue filters images without data and the subtree was never walked, so a failed capture left neither picture nor text. It now stops the export: unlike a float in a text flow, a capture that fails is a malfunction, not a property of PowerPoint.
- An identity transform keeps its element in the text flow. `translateZ(0)` and other painting hints resolve to the identity matrix and move nothing, but counted as out-of-flow — which cost a whole table its editable cells, since content needing its own box inside a cell rasterizes the table.
- The measured marker width is cached per slide instead of per process. Font faces are installed and removed around a render, so the same family name can resolve to a different font in the next one.
- A hanging `text-indent` is left alone when a block inside the shape carries its own left padding: one shape has one left inset, and that block would be laid out against the outer block's geometry.
...
- A wrapped anonymous text fragment keeps the width the browser wrapped it in. `range.getBoundingClientRect()` reports the union of the painted glyphs — for wrapped text the longest line, not the width it was broken in — and Office then broke it earlier, adding a line that pushed everything below it down. The width now comes from the content box of the fragment's block. A one-line fragment keeps its exact box as before: widening it would invite Office to make a second wrapping decision. This is the pattern that broke in production (the marker list of slide 9): its items ran 12.2 pt past their card and stranded a marker.
- Table cells no longer get margins wider than their CSS padding. `border-spacing` has no effect once the borders are collapsed, but the browser keeps reporting its value — 2px by default — and the converter added half of it to every cell edge. Nearly every table is collapsed, so nearly every table had cells that wrapped earlier than in the browser, which pushed the rows below down (17–18 pt on the fidelity fixture) and cost words at the column edge. The table probe is faithful now.
- Table rows carry the row height the browser laid out. PowerPoint treats it as a minimum and grows a row whose content needs more, so it cannot force text out of sight; without it every height was re-derived. This is not what fixed the table probe — the cell margins were, measured by mutation — and the fixture has no row the browser makes taller than its text, so the fidelity oracle does not yet prove it.
